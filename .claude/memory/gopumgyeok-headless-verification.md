...
- **실제 CMS API(`localhost:3001`, go_daepae.cms.api 개발 서버)에 의존하는 기능(팝업 목록
  ...
  타이밍으로 반복 재현 가능. CDP 탭을 열 때 `/json/new?URL`은 **PUT**으로 호출해야 한다
  (GET으로 부르면 405).

**2026-09-24 — 작은 아이콘을 확대해서 "이상해 보이는지" 판단할 때의 함정:**

- 사용자가 스크린샷 속 32px짜리 팝업 화살표 아이콘을 보고 "뭉개져 보인다"고 신고했을 때,
  `PIL.Image.resize(..., Image.NEAREST)`로 8배 확대해서 비교했더니 진짜로 지저분하게
  보였다 — 그런데 **그건 NEAREST(최근접 이웃) 보간 자체가 만든 확대 아티팩트였다.**
  같은 크롭을 `Image.LANCZOS`로 다시 확대하니 완전히 매끈한 화살표였고, CSS를 바꾸기 전/후
  버전 사이에 실제 시각 차이가 전혀 없었다(즉 처음 세운 "font-weight:700가 아이콘 폰트를
  합성 볼드로 뭉갠다" 가설은 틀렸다).
- **작은 UI 요소를 스크린샷에서 확대해 "깨졌는지" 판단할 때는 반드시 LANCZOS/BICUBIC 같은
  부드러운 보간으로 확대해야 한다.** NEAREST로 확대한 결과만 보고 실제 렌더링 버그로
  오판하면, 존재하지 않는 버그를 잡으려고 CSS를 고치게 된다.
- 이 오판 이후에도 실제 수정 방향(Swiper 기본 `swiper-icons` 폰트 글리프 대신 05 매장위치와
  같은 인라인 SVG chevron으로 교체)은 그대로 진행했다 — 폰트 렌더링은 브라우저/OS/버전마다
  힌팅이 달라 헤드리스 Chrome에서 깨끗해 보여도 사용자의 실제 환경(예: Safari)에서는 다르게
  보일 수 있고, 어차피 이 프로젝트는 이미 다른 곳(05 매장위치)에서 아이콘 폰트 대신 인라인
